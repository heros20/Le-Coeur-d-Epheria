...
/**
 * Joystick côté gauche : met à jour moveVector dans touchControlState
 * en fonction de la position du "thumb".
 */
function setupJoystick(root) {
  const joystickBase =
    root.querySelector("[data-joystick-base]") ||
    root.querySelector(".joystick-base");
  const joystickThumb =
    root.querySelector("[data-joystick-thumb]") ||
    root.querySelector(".joystick-handle");
  ...
  if (SUPPORTS_TOUCH_EVENTS) {
    ...
    ["touchend", "touchcancel"].forEach((evt) => {
      joystickBase.addEventListener(evt, handleTouchEnd, {
        passive: false,
      });
    });
  }
...
function setupTouchControls() {
  ...
  // Au boot : on garde caché, on ne l'affichera qu'une fois le jeu lancé
  root.classList.add("hidden");
  ...
  // Joystick gauche si présent
  setupJoystick(root);

  // Bouton Attaque (attaque mobile = combos / clic simulé)
  const attackBtn =
    root.querySelector("[data-touch-attack]") ||
  ...
  // Bouton Dash (juste un appui)
  const dashBtn =
    root.querySelector("[data-touch-dash]") ||
  ...
  // Interagir -> simule la touche "E"
  const interactBtn =
    root.querySelector("[data-touch-interact]") ||
    document.getElementById("btn-interact");
  if (interactBtn) {
    bindTouchButton(interactBtn, {
      onPress: () => setVirtualKey("e", true, true),
      onRelease: () => setVirtualKey("e", false),
    });
  }

  // Sprint -> simule "Shift"
  const sprintBtn =
    root.querySelector("[data-touch-sprint]") ||
    document.getElementById("btn-sprint");
  if (sprintBtn) {
    bindTouchButton(sprintBtn, {
      onPress: () => setVirtualKey("shift", true, false),
      onRelease: () => setVirtualKey("shift", false),
    });
  }
}
...
const PRINCESS_ANIMATION_SOURCES = {
  idle: [
    "./assets/Princesse/idle/Idle.png",
    "./assets/Princesse/idle/Idle_2.png",
  ],
...
function setupBoot() {
  ...
  // Pour l'instant : 1 seul héros, on le présélectionne
...
function sliceSheet(img) {
  ...
  const approx =
    frameH > 0 ? Math.max(1, Math.round(img.width / frameH)) : 1;
...
// ====== Auto-hide des barres fixes (mobile) ======
...
function markBottomFixedOverlays() {
  ...
  const cand = all.filter((el) => {
    ...
    // heuristique : barre large, collée en bas
    return (
      r.height >= 48 &&
      r.width >= 240 &&
...
async function startGame() {
  ...
  // prépare les contrôles mobiles (mais l'UI était encore cachée)
  if (State.isMobile) {
    resetTouchControlState();
    const root = document.getElementById("mobile-controls");
  ...
  // === Caméra adaptée à la taille de la map ===
  ...
  let kaelStart = world.nearestOpen(
    world.w * 0.86,
    world.h * 0.18,
    PLAYER_RADIUS
  );
  let princessPos = world.nearestOpen(
    world.w * 0.5,
    world.h * 0.5,
    PLAYER_RADIUS
  );
    ...
    State.pickups = State.pickups.filter((pickup) => {
      ...
      if (dist <= pickup.radius + player.r + 4) {
        if (pickup.type === "potion") {
          const added = State.inventory.add(
            pickupFactory.potion()
          );
          if (added) {
            pushStatus("Potion ajoutée");
            return false;
          }
  ...
  function update(dt) {
    ...
    const attackPressed = State.isMobile
      ...
      : consumePointer(0);
    const pointerHeld = State.isMobile
      ? mobileAttackHeld()
      : pointerDown(0);
    ...
    let attackHoldTime =
      pointerHeld && attackState.wasHeld
        ? State.time - attackState.holdStart
        : 0;
    let attackDoubleTap = false;

    if (attackPressed) {
      const window =
        CONFIG.comboDoubleTapWindow ?? 0.28;
      attackState.pendingDouble =
        State.time - attackState.lastTap <= window;
    ...
    if (consume("r")) {
      State.mode =
        State.mode === "LIGHT" ? "SHADOW" : "LIGHT";
    }

    const dashPressed =
      consume(" ") ||
      (State.isMobile && consumeMobileDashPress());
    ...
    if (!State.dialogue.isOpen() && interactPressed) {
      tryInteract();
    }
    ...
    if (dashPressed) {
      ...
      const pointerVector = pointerData
        ? {
            x: pointerData.x - player.x,
            y: pointerData.y - player.y,
          }
        : null;
      const pointerValid =
        pointerVector &&
        (Math.abs(pointerVector.x) > 0.01 ||
          Math.abs(pointerVector.y) > 0.01);
      const moveVectorValid =
        moveVectorInput &&
        (Math.abs(moveVectorInput.x) > 0.01 ||
          Math.abs(moveVectorInput.y) > 0.01);
    ...
    // Logique Kael / princesse / trahison
    if (
      !State.flags.betrayalHappened &&
      State.kael.follow &&
      Math.hypot(
        player.x - State.princess.x,
        player.y - State.princess.y
      ) < 110 &&
    ...
    if (
      ...
      if (
        ...
        Math.hypot(
          player.x - State.boss.x,
          player.y - State.boss.y
        ) < attackRange &&
        player.isTargetInAttackArc?.(
          State.boss.x,
          State.boss.y
        )
      ) {
        const dmg =
          player.getCurrentAttackDamage?.() ?? 15;
        State.boss.hit(dmg);
      ...
      if (!State.boss.alive) {
        ...
        State.dialogue.show([
          {
            speaker: "Mur",
            text: "Le jugement approche. Ramène Aelya à la porte.",
          },
        ]);
      }
    }
  ...
  function tryUsePotion() {
    const used = State.inventory.use("potion", {
      player: State.player,
      notify: pushStatus,
    });
    if (!used) pushStatus("Pas de potion disponible");
  }

  function tryInteract() {
    const dKael = Math.hypot(
      State.player.x - State.kael.x,
      State.player.y - State.kael.y
    );
    ...
    if (dKael < 70 && !State.flags.kaelMet) {
      ...
      State.dialogue.show([
        {
          speaker: "Kael",
          text: "Tu as franchi les arches. Tes choix te collent à la peau.",
        },
        {
          speaker: "Kael",
          text: "Je marcherai à tes côtés, mais sache que chaque pas réclame un tribut.",
        },
        {
          speaker: "Kael",
          text: "Allons vers la princesse et regarde ce que tes décisions provoquent.",
        },
      ]);
      return;
    }
    ...
    const dP = Math.hypot(
      State.player.x - State.princess.x,
      State.player.y - State.princess.y
    );
    if (dP < 60 && !State.princess.follow) {
      if (!State.flags.kaelDefeated) {
        State.dialogue.show([
          {
            speaker: "Princesse",
            text: "Kael tient encore... débarrasse-toi de lui !",
          },
        ]);
        return;
      }
  ...
  function triggerBetrayal() {
    ...
    State.dialogue.show([
      {
        speaker: "Kael",
        text: "Te voilà face à son innocence. Chaque décision porte une ombre.",
      },
      {
        speaker: "Kael",
        text: "Je suis le prix de tes choix, Lioran. Tu ne quitteras pas ce labyrinthe indemne.",
      },
      {
        speaker: "Kael",
        text: "Prépare-toi à affronter les conséquences.",
      },
    ]);
  }
  ...
  function render() {
    ...
    if (!State.flags.betrayalHappened) {
      drawHitCircle(
        State.kael,
        State.kael?.hitRadius,
        "#FFD966"
      );
    }
    drawHitCircle(
      State.princess,
      State.princess?.hitRadius,
      "#A5B4FC"
    );
    if (
      State.flags.betrayalHappened &&
  ...
  function renderBossGameOver() {
    ...
    el
      .querySelector("[data-retry-boss]")
      ?.addEventListener("click", () => retryBossFight());
    el
  ...
  function retryBossFight() {
    ...
    const bossSpawn = checkpoint?.boss
      ? { x: checkpoint.boss.x, y: checkpoint.boss.y }
      : undefined;
